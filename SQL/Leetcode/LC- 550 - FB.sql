...
that logged in for at least two consecutive days starting from their first login date, then divide that number 
...
select convert(decimal(3,2),A.player_id/C.TotalUsers) FRACTION
from Activity A LEFT OUTER JOIN 
(select player_id, min(event_date) as FirstLogin 
from  Activity group by player_id) B  ON A.player_id = B.player_id
, (select convert(decimal(3,2),count(distinct player_id)) TotalUsers from Activity)  C
where DATEDIFF(day,FirstLogin,event_date) = 1
